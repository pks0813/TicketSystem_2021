...
class core{
    ...
    int Usernumber=0;
    int Ordertime=0;
    core():UserBPT("UserBPT.in"),TrainBPT("TrainBPT.in"),StationTrainBPT("StationTrainBPT.in"),UserTimeBPT("UserTimeBPT.in"),
            ...
            Userpool("Userpool.in",8000)/*1M*/,Trainpool("Trainpool.in",10000)/*14M*/,Orderpool("Orderpool.in",10000)/*2M*/,Seatpool("Seatpool.in",10000)/*6M*/,Stationpool("Stationpool.in",2500)/*6M*/,Useronline(10007),MPS(107)
    ...
    void game(){
        ...
        while (1){
            ...
            if (tmp[0]=="add_user"){add_user();} 
            if (tmp[0]=="login"){login();}
            if (tmp[0]=="logout"){logout();}
            if (tmp[0]=="query_profile"){query_profile();}
            if (tmp[0]=="modify_profile"){modify_profile();}
            if (tmp[0]=="add_train"){add_train();}
            if (tmp[0]=="release_train"){release_train();}
            if (tmp[0]=="query_train"){query_train();}
            if (tmp[0]=="delete_train"){delete_train();}
            if (tmp[0]=="query_ticket"){query_ticket();} 
            if (tmp[0]=="query_transfer"){query_transfer();}
            if (tmp[0]=="buy_ticket"){buy_ticket();}
            if (tmp[0]=="query_order"){query_order();}
            if (tmp[0]=="refund_ticket"){refund_ticket();}
