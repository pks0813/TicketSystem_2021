...
#ifndef TICKETSYSTEM_2021_BPT_SUBMIT_HPP
#define TICKETSYSTEM_2021_BPT_SUBMIT_HPP
#include <iostream>
...
public:
    BPT(const std::string &f_n) :storage(f_n){
     ...
     */
    int IntervalFind(const Key &left, const Key &right, int a[]){
        if (root == -1) return 0;
        bpt_node node;
        int sub_tree;
        storage.Copy(root, node);
